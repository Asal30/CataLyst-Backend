...
def _detect_ring_via_contours(gray: np.ndarray):
    H, W = gray.shape[:2]
    cx0, cy0 = W // 2, H // 2
...
def _detect_ring_via_hough(gray: np.ndarray):
    H, W = gray.shape[:2]
    cx0, cy0 = W // 2, H // 2
...
def _detect_ring_via_edge_hough(gray: np.ndarray):
    H, W = gray.shape[:2]
    cx0, cy0 = W // 2, H // 2
...
def _detect_white_circle_with_dark_ring(original_rgb: np.ndarray):
    gray = cv2.cvtColor(original_rgb, cv2.COLOR_RGB2GRAY)
    gray = cv2.GaussianBlur(gray, (9, 9), 1.5)
...
def _apply_colormap_to_cam(cam: np.ndarray, size_hw: tuple[int, int]) -> np.ndarray:
    h, w = size_hw
    cam_resized = cv2.resize(cam, (w, h), interpolation=cv2.INTER_LINEAR)
    cam_uint8 = np.uint8(np.clip(cam_resized, 0.0, 1.0) * 255.0)
    heatmap_bgr = cv2.applyColorMap(cam_uint8, cv2.COLORMAP_JET)
    heatmap_rgb = cv2.cvtColor(heatmap_bgr, cv2.COLOR_BGR2RGB)
    return heatmap_rgb


def _blend_heatmap_with_image(original_rgb: np.ndarray, heatmap_rgb: np.ndarray, alpha: float = 0.38) -> np.ndarray:
    return cv2.addWeighted(original_rgb, 1.0 - alpha, heatmap_rgb, alpha, 0)


def _build_heuristic_overlay(original_rgb: np.ndarray):
    ...
    overlay_rgb = original_rgb.copy()
    fill = overlay_rgb.copy()
    cv2.circle(fill, (circle["cx"], circle["cy"]), circle["r"], (255, 0, 0), thickness=-1)
    overlay_rgb = cv2.addWeighted(overlay_rgb, 0.78, fill, 0.22, 0)
    cv2.circle(overlay_rgb, (circle["cx"], circle["cy"]), circle["r"], (255, 0, 0), thickness=5)

    return overlay_rgb, circle, circle_meta
...
def generate_cbm_concept_gradcams(image_array: np.ndarray, model: torch.nn.Module,
    ...
    try:
        ...
        center_prior = _build_center_prior(cam.shape[0], cam.shape[1])
        cam_with_prior = np.clip(0.75 * cam + 0.25 * center_prior, 0.0, 1.0)

        original_rgb = np.array(pil_img.convert("RGB"), dtype=np.uint8)
        H, W = original_rgb.shape[:2]

        raw_heatmap_rgb = _apply_colormap_to_cam(cam, (H, W))
        model_overlay_rgb = _blend_heatmap_with_image(original_rgb, raw_heatmap_rgb, alpha=0.38)

        prior_heatmap_rgb = _apply_colormap_to_cam(cam_with_prior, (H, W))
        prior_overlay_rgb = _blend_heatmap_with_image(original_rgb, prior_heatmap_rgb, alpha=0.38)

        heuristic_overlay_rgb, circle, circle_meta = _build_heuristic_overlay(original_rgb)

        raw_heatmap_fname = f"raw_heatmap_cbm_{run_id}.jpg"
        raw_heatmap_path = os.path.join(OUTPUT_DIR, raw_heatmap_fname)
        _save_image(raw_heatmap_rgb, raw_heatmap_path)

        model_overlay_fname = f"gradcam_cbm_{run_id}.jpg"
        model_overlay_path = os.path.join(OUTPUT_DIR, model_overlay_fname)
        _save_image(model_overlay_rgb, model_overlay_path)

        prior_overlay_fname = f"gradcam_center_prior_cbm_{run_id}.jpg"
        prior_overlay_path = os.path.join(OUTPUT_DIR, prior_overlay_fname)
        _save_image(prior_overlay_rgb, prior_overlay_path)

        heuristic_overlay_fname = f"heuristic_overlay_cbm_{run_id}.jpg"
        heuristic_overlay_path = os.path.join(OUTPUT_DIR, heuristic_overlay_fname)
        _save_image(heuristic_overlay_rgb, heuristic_overlay_path)

        raw_heatmap_url = _url_for_path(raw_heatmap_path)
        model_overlay_url = _url_for_path(model_overlay_path)
        prior_overlay_url = _url_for_path(prior_overlay_path)
        heuristic_overlay_url = _url_for_path(heuristic_overlay_path)

        return {
            "gradcam_path": model_overlay_url,
            "raw_heatmap_path": raw_heatmap_url,
            "center_prior_gradcam_path": prior_overlay_url,
            "heuristic_overlay_path": heuristic_overlay_url,

            "gradcam_paths": {dominant_name: model_overlay_url},
            "heatmap_paths": {dominant_name: raw_heatmap_url},
            "heuristic_overlay_paths": {dominant_name: heuristic_overlay_url},
            ...
            "visual_method": {
                "primary_explanation": "raw_gradcam_overlay",
                "supporting_views": [
                    "raw_heatmap",
                    "center_prior_gradcam_overlay",
                    "heuristic_circle_overlay"
                ],
                "note": "Heuristic overlay is auxiliary and should not be interpreted as the direct model explanation."
            },
        }
...
def generate_gradcam_from_image_array(image_array: np.ndarray, model: torch.nn.Module, output_name: str = None) -> str | None:
    result = generate_cbm_concept_gradcams(image_array, model)
    if result.get("gradcam_path"):
        return result["gradcam_path"]
